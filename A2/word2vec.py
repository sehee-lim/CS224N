...
def naiveSoftmaxLossAndGradient(
    ...
    y_hat[outsideWordIdx] -= 1   # y_hat - y
...
def negSamplingLossAndGradient(
    ...
    loss -= np.log(z)
    gradCenterVec = -u_o * (1 - z)
    gradOutsideVecs[outsideWordIdx] -= (1 - z) * centerWordVec
    
    for idx in negSampleWordIndices:
        u_s = outsideVectors[idx]
        z = sigmoid(-np.dot(u_s, centerWordVec))
        loss -= np.log(z)
        gradCenterVec += u_s * (1 - z)
        gradOutsideVecs[idx] += (1 - z) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        l, gradCenter, gradOutside = word2vecLossAndGradient(
            centerWordVectors[centerWordIdx], word2Ind[outsideWord], outsideVectors, dataset
        )
        loss += l
